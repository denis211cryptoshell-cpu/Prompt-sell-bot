"""Main user-facing keyboards — i18n aware."""
...
def main_menu_kb(lang: str = "ru") -> InlineKeyboardMarkup:
    ...
    builder.button(text=t("btn_language", lang), callback_data="menu:language")
    builder.adjust(1)
    return builder.as_markup()


def product_kb(product: Product, already_purchased: bool = False, lang: str = "ru") -> InlineKeyboardMarkup:
    """Product view keyboard."""
    builder = InlineKeyboardBuilder()
    if already_purchased:
        builder.button(text=t("btn_buy_already", lang), callback_data=f"purchase:download:{product.id}")
    else:
...
def pay_with_robokassa_kb(pay_url: str, product_id: int, lang: str = "ru") -> InlineKeyboardMarkup:
    """Keyboard with URL button to open Robokassa payment page."""
    builder = InlineKeyboardBuilder()
    builder.button(text=t("btn_pay_robokassa", lang), url=pay_url)
    builder.button(text=t("btn_back_to_product", lang), callback_data="catalog:view")
    builder.adjust(1)
    return builder.as_markup()
